...
router.get("/shampoing", getProduits, (req, res) => {
  ...
  res.status(200).render(path.join(__dirname, "../views/shampoing.ejs"), {
    isConnected,
    detailsProducts,
  });
});

router.get("/apresShampoing", getProduits, (req, res) => {
  // Gestion de la requête GET pour l'endpoint "/produit"
  ...
  console.log(detailsProducts);
  // Envoi d'une réponse avec le code d'état 200 et rendu de la vue "produit.ejs"
  res.status(200).render(path.join(__dirname, "../views/apresShampoing.ejs"), {
    isConnected,
    detailsProducts,
  });
});
...
router.get("/huile", getProduits, (req, res) => {
  // Gestion de la requête GET pour l'endpoint "/produit"
  ...
  console.log(detailsProducts);
  // Envoi d'une réponse avec le code d'état 200 et rendu de la vue "produit.ejs"
  res.status(200).render(path.join(__dirname, "../views/huile.ejs"), {
    isConnected,
    detailsProducts,
  });
});
